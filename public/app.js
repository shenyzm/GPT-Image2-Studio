...
import { buildFailedJobPatch, buildFailedPreviewNodes, buildRetryJobPatch } from "/lib/failed-job-actions.mjs";
import { toggleSelection, selectAll, formatSelectionCountLabel, formatMultiSelectButtonLabel, formatBatchDeleteConfirm, resolveSelectedItems } from "/lib/gallery-selection.mjs";
...
const refs = {
  ...
  galleryMultiSelectButton: document.querySelector("#galleryMultiSelectButton"),
  galleryBatchActions: document.querySelector("#galleryBatchActions"),
  gallerySelectAllButton: document.querySelector("#gallerySelectAllButton"),
...
function getQueuedJobCount() {
  return state.jobs.filter((job) => !job.failed).length;
}

function getRunningJobCount() {
  return state.jobs.filter((job) => job.isRunning && !job.failed).length;
}
...
function renderPreviewPlaceholder(placeholderState, item = null) {
  ...
  if (placeholderState.mode === "failed") {
    refs.previewPlaceholder.classList.add("preview-placeholder-failed");
    buildFailedPreviewNodes(placeholderState, item, {
      onRetry: retryJob,
      onDismiss: (jobId) => {
        const job = state.jobs.find((entry) => entry.id === jobId);
        if (!job) return;
        if (state.selectedPreviewKey === makeJobPreviewKey(jobId)) {
          state.selectedPreviewKey = "";
        }
        if (job.mode === "reference-analysis") {
          removeReferenceAnalysisGenerationKey(makeJobPreviewKey(jobId));
        }
        if (job.mode === "image-decomposition") {
          removeImageDecompositionGenerationKey(makeJobPreviewKey(jobId));
        }
        if (job.mode === "quick-blend") {
          removeQuickBlendGenerationKey(makeJobPreviewKey(jobId));
        }
        removeJob(jobId);
        renderAll();
      },
    }).forEach((node) => refs.previewPlaceholder.appendChild(node));
    return;
  }
...
function renderPreview() {
  ...
  if (placeholderState.mode === "failed") {
    refs.previewPlaceholder.classList.remove("hidden");
    renderPreviewPlaceholder(placeholderState, item);
    refs.previewImage.removeAttribute("src");
    refs.previewImage.classList.remove("is-mounted", "is-visible");
    refs.previewDownloadButton.removeAttribute("href");
    refs.previewDownloadButton.removeAttribute("download");
...
function syncFilmstripMedia(button, item) {
  ...
  ghost.className = item?.failed ? "filmstrip-ghost filmstrip-ghost-failed" : "filmstrip-ghost";
  ghost.textContent = item?.failed ? "失败" : "处理中";
  if (!existingGhost) {
...
function syncFilmstripRetryButton(shell, key, item) {
  const existingRetryButton = shell.querySelector(".filmstrip-retry");
  const shouldRenderRetryButton = key.startsWith("job:") && Boolean(item?.failed);
  if (!shouldRenderRetryButton) {
  ...
  retryButton.className = "filmstrip-retry";
  retryButton.textContent = "↻";
  retryButton.title = "重试生成";
  retryButton.setAttribute("aria-label", "重试生成");
  retryButton.addEventListener("click", (event) => {
    event.stopPropagation();
    retryJob(item.id);
  });
  shell.appendChild(retryButton);
}
...
function syncFilmstripEntry(shell, { key, item, label }) {
  ...
  button.classList.toggle("failed", Boolean(item?.failed));
  ...
  syncFilmstripRetryButton(shell, key, item);
}
...
function createGalleryTile(item) {
  ...
  button.dataset.galleryFilename = item.filename;

  if (state.galleryMultiSelectMode) {
    button.classList.toggle("selected", state.gallerySelection.has(item.filename));
    button.addEventListener("click", () => {
      toggleSelection(state.gallerySelection, item.filename);
      renderGalleryView();
    });

    const checkbox = document.createElement("div");
    checkbox.className = "gallery-tile-checkbox";
    checkbox.innerHTML = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M7 10l2 2 4-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function toggleGalleryMultiSelectMode() {
  state.galleryMultiSelectMode = !state.galleryMultiSelectMode;
  if (!state.galleryMultiSelectMode) {
    state.gallerySelection.clear();
  }
  refs.galleryMultiSelectButton.textContent = formatMultiSelectButtonLabel(state.galleryMultiSelectMode);
  refs.galleryBatchActions.classList.toggle("hidden", !state.galleryMultiSelectMode);
}
...
async function deleteSelectedGalleryItems() {
  ...
  if (!confirm(formatBatchDeleteConfirm(state.gallerySelection.size))) {
    return;
  }
...
function renderGalleryView() {
  ...
  if (state.galleryMultiSelectMode) {
    refs.gallerySelectionCount.textContent = formatSelectionCountLabel(state.gallerySelection.size);
  }
...
// Mark a job as failed instead of removing it, so the card stays visible
// with a retry affordance. The job keeps all of its original parameters
// (prompt, ratio, size, reference files, ...) so it can be re-run as-is.
function failJob(jobId, message) {
  ...
  Object.assign(job, buildFailedJobPatch(message));
  // Surface the failed job in the preview panel so the retry button is visible.
  state.selectedPreviewKey = makeJobPreviewKey(job.id);
  return job;
}

// Re-run a failed job using its preserved parameters. Resets it back to a
// queued state so the normal scheduler picks it up again.
function retryJob(jobId) {
  ...
  Object.assign(job, buildRetryJobPatch());
  renderAll();
  scheduleGenerationQueue();
}
...
function applyGenerationTaskSnapshots(tasks, { render = true } = {}) {
  ...
  snapshots.forEach((task) => {
    ...
    if (task.status === "error" && wasSelectedPreview && !existingJobs.has(task.id)) {
      state.selectedPreviewKey = "";
    }
  ...
  // Server-side failures: keep the originating local job (with its reference
  // files and parameters) as a failed card so it can be retried.
  const remoteFailedJobs = snapshots
    .filter((task) => task.status === "error" && existingJobs.has(task.id))
    .map((task) => {
      const existing = existingJobs.get(task.id);
      const message = compactErrorMessage(task.errorMessage, "生成任务失败");
      return {
        ...existing,
        failed: true,
        isRunning: false,
  ...
  const remoteFailedIds = new Set(remoteFailedJobs.map((job) => job.id));
  const localTransientJobs = state.jobs.filter(
    (job) =>
      !remoteFailedIds.has(job.id) &&
      !snapshotIds.has(job.id) &&
      (job.isRunning || !job.started || job.failed),
  );

  state.generationTasks = snapshots;
  state.jobs = sortGalleryItemsByCreatedAtDesc([...remoteRunningJobs, ...remoteFailedJobs, ...localTransientJobs]);
...
function cleanupModeOnFailure(job, message) {
  const previewKey = makeJobPreviewKey(job.id);
  if (job.mode === "reference-analysis") {
    removeReferenceAnalysisGenerationKey(previewKey);
  }
  if (job.mode === "image-decomposition") {
    removeImageDecompositionGenerationKey(previewKey);
    setImageDecompositionFeedback(message, "error");
  }
  if (job.mode === "quick-blend") {
    removeQuickBlendGenerationKey(previewKey);
...
async function runGeneration(job) {
  ...
  try {
    ...
    if (!response) {
      const message = "生成请求多次重试后仍失败，请稍后重试。";
    ...
    await consumeSse(response.body, async (eventName, payload) => {
      ...
      if (eventName === GENERATION_STREAM_EVENTS.ERROR) {
        ...
        handleActivityFailure(job.id, message);
        showError(message);
        cleanupModeOnFailure(job, message);
        failJob(job.id, message);
        renderAll();
      }
    });
...
function bindEvents() {
  ...
  refs.galleryMultiSelectButton.addEventListener("click", () => {
    toggleGalleryMultiSelectMode();
    renderGalleryView();
  });
  refs.gallerySelectAllButton.addEventListener("click", () => {
    selectAll(state.gallerySelection, getVisibleGalleryItems());
    renderGalleryView();
  });
  refs.galleryClearSelectionButton.addEventListener("click", () => {
    state.gallerySelection.clear();
    renderGalleryView();
  });
  refs.galleryDeleteSelectedButton.addEventListener("click", () => {
    void deleteSelectedGalleryItems();
  });
  refs.galleryColumnButtons.forEach((button) => {
    button.addEventListener("click", () => {
