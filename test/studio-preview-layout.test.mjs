...
const sharedNormalizationPath = new URL("../lib/shared/normalization.mjs", import.meta.url);
...
test("quick blend mode implements upload pairing queue preview and cleanup contracts", async () => {
  ...
  assert.match(app, /function cleanupModeOnFailure\(job, message\) \{[\s\S]*if \(job\.mode === "quick-blend"\) \{[\s\S]*removeQuickBlendGenerationKey/);
  assert.match(app, /setQuickBlendFeedback\(message, "error"\);/);
...
test("reference analysis generation mode survives task polling snapshots", async () => {
  const app = await readFile(appPath, "utf8");
  const server = await readFile(serverPath, "utf8");
  const sharedNormalization = await readFile(sharedNormalizationPath, "utf8");
  ...
  assert.match(sharedNormalization, /const GENERATION_MODES = new Set\(\[[\s\S]*"style-transfer"[\s\S]*"reference-analysis"[\s\S]*IMAGE_DECOMPOSITION_MODE[\s\S]*\]\);/);
  assert.match(sharedNormalization, /function normalizeGenerationMode\(value\) \{[\s\S]*GENERATION_MODES\.has\(mode\) \? mode : "";/);
