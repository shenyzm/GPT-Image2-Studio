...
const sharedReferenceLabelsPath = new URL("../lib/shared/reference-labels.mjs", import.meta.url);
...
test("portrait runtime labels clothing references as mandatory wardrobe authorities", async () => {
  const sharedReferenceLabels = await readFile(sharedReferenceLabelsPath, "utf8");

  assert.match(sharedReferenceLabels, /Portrait clothing, prop, and accessory reference/);
  assert.match(sharedReferenceLabels, /WARDROBE LOCK/);
  assert.match(sharedReferenceLabels, /must wear the supplied outfit/);
  assert.match(sharedReferenceLabels, /Do not replace it with a generic blazer, suit, dress, or everyday outfit/);
  assert.match(sharedReferenceLabels, /do not treat it as another person identity/);
});
