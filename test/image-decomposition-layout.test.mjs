...
test("image decomposition mode is exposed as an independent Create view", async () => {
  ...
  assert.match(app, /function cleanupModeOnFailure\(job, message\) \{[\s\S]*if \(job\.mode === "image-decomposition"\) \{[\s\S]*removeImageDecompositionGenerationKey/);
  assert.match(app, /setImageDecompositionFeedback\(message, "error"\);/);
