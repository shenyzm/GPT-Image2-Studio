...
const sharedImageUtilsPath = new URL("../lib/shared/image-utils.mjs", import.meta.url);
...
test("creation generation accepts references image count marketing scenario and industry template", async () => {
  const server = await readFile(serverPath, "utf8");
  const worker = await readFile(cloudflareWorkerPath, "utf8");
  const sharedImageUtils = await readFile(sharedImageUtilsPath, "utf8");

  // The logoImage form field is read inside the shared readCreationLogoImage helper.
  assert.match(sharedImageUtils, /formData\.getAll\("logoImage"\)/);
  ...
  assert.match(server, /readCreationLogoImage\(formData\)/);
  ...
  assert.match(worker, /readCreationLogoImage\(formData\)/);
